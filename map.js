...
map.on("load", function () {
  ...
  //     var population = e.features[0].properties.pop2;

  //     // Format the population number (optional)
  //     var formattedPopulation = population.toLocaleString();

  //     new mapboxgl.Popup()
  //         .setLngLat(e.lngLat)
  //         .setHTML(`<strong>${countryName}</strong><br>Population: ${formattedPopulation}`)
  //         .addTo(map);
  // });
  ...
  map.addLayer({
    ...
    source: {
        type: "geojson",
        data: "data/wna_hub_countries.geojson",
    },
    paint: {
...
map.addLayer({
  ...
  source: {
      type: "geojson",
      data: "data/wna_coutries_rdpr5.geojson",
  },
  paint: {
      'fill-color': [
          'match',
          ['get', 'PriorityMappingCategory'],
          'Immediate Action', '#8B0000',   // Darkest red
          'Medium Priority', '#E2725B', // Medium red
          'Lower Priority', '#F4A460',    // Light red (Tomato color)
          '#CCCCCC'            // Default color (gray) for missing values
      ],
      'fill-opacity': 1
  }
});
...
map.addLayer({
  ...
  //     var countryName = e.features[0].properties.Country;
  //     var affected = e.features[0].properties.affec;

  //     // Format the population number (optional)
  //     var formattedAffected = affected.toLocaleString();

  //     new mapboxgl.Popup()
  //         .setLngLat(e.lngLat)
  //         .setHTML(`<strong>${countryName}</strong><br>Population: ${formattedAffected}`)
  //         .addTo(map);
  // });
